...
void push_heap(RandomAccessIterator first, RandomAccessIterator last, RandomAccessIterator element,
               Compare cmp = {})
{
    ...
    auto next_location = (not cmp(*left_child, *right_child)) ? left_child : right_child;
    std::iter_swap(next_location, element);
    shino::push_heap(first, last, next_location, cmp);
}

// use bottom up heap construction of max-heap
template <typename RandomAccessIterator, typename Compare = std::less<>>
void build_heap(RandomAccessIterator first, RandomAccessIterator last, Compare cmp = {})
{
    for (auto middle = first + std::distance(first, last) / 2; middle != first; --middle)
    {
